...
| **Render frame (1:1)** | Outline of the render area, plus its colour (default yellow). |
...
1. `rcm_field()` works out, in screen pixels, the rectangle matching the render
   area inside the viewport — same logic as the safe frame. On a 1:1 render, that
   is a centred square.
2. Inside it, the **target ratio rectangle** (`W/H`) is computed at full width
   (landscape ratio) or full height (portrait ratio), then the **offset** is
   applied on whichever axis has slack.
3. The **matte** is painted (4 bands around the rectangle), then the square
   outline, the ratio outline, the rule of thirds and the ratio label.
4. UI handlers update global variables and fire a single `completeRedraw()` — no
...
- State globals: `RCM_active`, `RCM_aspect`, `RCM_label`, `RCM_showMask`,
  `RCM_maskColor`, `RCM_showFrame`, `RCM_frameColor`, `RCM_showThirds`,
  `RCM_cropColor`, `RCM_offset`.
